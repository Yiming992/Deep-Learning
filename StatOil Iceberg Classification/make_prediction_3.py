...
test_images=np.load('test_images_2.pk')
test_angle=np.load('test_angle_2.pk')
...
with tf.Session(graph=loaded_graph) as sess:
    loader=tf.train.import_meta_graph('./model_save_2/CK.meta')

    loader.restore(sess, tf.train.latest_checkpoint('./model_save_2/'))
...
submit.to_csv('submit_34.csv',index=False)
