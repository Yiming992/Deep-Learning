...
train_images=np.load('train_img.pk')
train_label=np.load('train_lab.pk')
...
with main_graph.as_default():
    ...
    with tf.name_scope('inception_block_4'):
        incep_4=inception_block(incep_1,64,64,64,64,64,64,64,is_train)
    ...
    #     max_pool_3=tf.layers.dropout(max_pool_3,rate=0.4,training=is_train)
    with tf.name_scope('global_max_pooling'):
        g_max_pool=tf.reduce_mean(incep_4,[1,2])
