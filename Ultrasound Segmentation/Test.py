...
if __name__=='__main__':
    ...
    with tf.Session(graph=loaded_graph) as sess:
        ...
        mask=loaded_graph.get_tensor_by_name('final_layer/Cast:0')
        for i in range(len(test_images)):
            test_img=cv2.imread(test_images[i])
            test_img=test_img[:,:,0]
            resized_img=cv2.resize(test_img,(80,112),interpolation=cv2.INTER_AREA)
            resized_img=resized_img[np.newaxis,:,:,np.newaxis]
            Mask=sess.run(mask,feed_dict={X:resized_img})
            Mask=np.squeeze(Mask)
            Mask=cv2.resize(Mask*255.0,(420,580),interpolation=cv2.INTER_AREA)
