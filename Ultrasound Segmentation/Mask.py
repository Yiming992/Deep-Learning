...
with Graph.as_default():
    ...
    mask=tf.placeholder(tf.int32,[None,None,None,1])
    ...
    with tf.name_scope('final_layer'):
        ...
        Final_mask=tf.cast(Final>0.5,dtype=tf.float32)
...
if __name__=='__main__':
    ...
    def get_batch(images,masks,batch_size,seed=0):
        ...
        for i in range(len(images)//batch_size):
            ...
            for i in range(sub_images.shape[0]):
                ...
                imgs=imgs[0]
                labels=labels[0]
                imgs=cv2.resize(imgs,(80,112),interpolation=cv2.INTER_AREA)
                labels=cv2.resize(labels,(80,112),interpolation=cv2.INTER_AREA)
                img.append(imgs)
                lab.append(labels)
            yield np.stack(img),np.stack(lab)
    ...
    with tf.Session(graph=Graph) as sess:
        ...
        for i in range(num_epochs):
            for img,lab in get_batch(train_images,train_mask,batch_size,seed=i):
                iter+=1
                img=np.expand_dims(img,axis=-1)
                lab=np.expand_dims(lab,axis=-1)
                sess.run(train_op,feed_dict={Input:img/255.0,mask:lab/255.0})
                if iter%10==0:
